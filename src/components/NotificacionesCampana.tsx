...
const NotificacionesCampana: React.FC = () => {
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      {mostrarPanel && (
        <div className="absolute z-[999999] right-0 mt-3 w-[400px] bg-[#F8F9FA] dark:bg-[#0F1219] rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] dark:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] border border-gray-100 dark:border-slate-700 z-50 overflow-hidden font-sans">
          {/* Header Limpio */}
              ...
              notificaciones?.slice(0, 5).map((notificacion) => {
                ...
                if (sunatLink) {
                  return (
                    <Link
                      ...
                      className={`block bg-white dark:bg-slate-900/80 p-3 rounded-xl shadow-sm border hover:border-gray-300 dark:hover:border-slate-600 transition-all relative group ${!notificacion.leida ? 'border-red-200 dark:border-red-900/50' : 'border-transparent dark:border-slate-800'}`}
                    >
                      {cardContent}
                    </Link>
                  );
                ...
                return (
                  <div
                    ...
                    className={`bg-white dark:bg-slate-900/80 p-3 rounded-xl shadow-sm border border-transparent dark:border-slate-800 hover:border-gray-200 dark:hover:border-slate-600 transition-all cursor-pointer relative group`}
                  >
                    {cardContent}
                  </div>
                );
