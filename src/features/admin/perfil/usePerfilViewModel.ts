...
export const usePerfilViewModel = () => {
    ...
    const [savingSunatValidez, setSavingSunatValidez] = useState(false);
    ...
    const handleImpresionConfig = async (
        campo: 'mostrarQrSunat' | 'formatoImpresionDefault' | 'imprimirAutomatico' | 'mostrarMarcaSistema',
        ...
        try {
            ...
            const mensajes: Record<string, string> = {
                ...
                mostrarMarcaSistema: valor
                    ? 'La marca del sistema volverá a salir al pie de tus comprobantes'
                    : 'La marca del sistema ya no saldrá en tus comprobantes',
    ...
    const handleCobranzaCampoToggle = async (enabled: boolean) => {
        if (savingCobranzaCampoConfig || cobranzaCampoToggleInFlight.current) return;
        if (Boolean(perfil?.empresa?.cobranzaCampo) === enabled) return;
        try {
            cobranzaCampoToggleInFlight.current = true;
            setSavingCobranzaCampoConfig(true);
            await useEmpresasStore.getState().actualizarMiEmpresa({ cobranzaCampo: enabled });
            setPerfil(prev => (prev ? { ...prev, empresa: { ...prev.empresa, cobranzaCampo: enabled } } : prev));
            useAuthStore.setState(state => ({
                auth: state.auth ? { ...state.auth, empresa: { ...(state.auth as any).empresa, cobranzaCampo: enabled } } : state.auth,
            }));
            useAlertStore.getState().alert('Configuración de cobranza en campo actualizada', 'success');
        } catch (error: any) {
            useAlertStore.getState().alert(error?.response?.data?.message || error?.message || 'No se pudo actualizar la configuración', 'error');
        } finally {
            cobranzaCampoToggleInFlight.current = false;
            setSavingCobranzaCampoConfig(false);
        }
    };
    ...
    const handleControlFlagToggle = async (
        flag: 'requiereAprobacionGastos' | 'requiereAprobacionCompras' | 'requiereCajaParaEmitir' | 'catalogoPorSede' | 'kitsComoUnaLinea' | 'paquetesComoUnaLinea' | 'posMantenerBusqueda' | 'posExigirCpeMedioPago',
        enabled: boolean,
    ) => {
    ...
    const handleSunatValidezSave = async (
        clientId: string,
        clientSecret: string,
        onDone?: () => void,
    ) => {
        if (savingSunatValidez) return;
        try {
            setSavingSunatValidez(true);
            await useEmpresasStore.getState().actualizarMiEmpresa({
                sunatClientId: clientId,
                sunatClientSecret: clientSecret,
            } as any);
            setPerfil(prev => {
                if (!prev) return prev;
                return { ...prev, empresa: { ...prev.empresa, sunatClientId: clientId, sunatClientSecret: clientSecret } };
            });
            onDone?.();
            useAlertStore.getState().alert('Credenciales de Consulta de Validez SUNAT guardadas', 'success');
        } catch (error: any) {
            useAlertStore.getState().alert(error?.response?.data?.message || error?.message || 'No se pudieron guardar las credenciales', 'error');
        } finally {
            setSavingSunatValidez(false);
        }
    };
