                  ...
                  {COTIZ_ELEMENTOS.filter((e) => e.grupo === g && (!e.soloEn || e.soloEn.includes(configKey))).map((el) => {
                    ...
                    return (
                      <div key={el.key} className="p-2.5 rounded-xl border border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/40">
                        ...
                        <span className={`flex-1 text-sm ${cur.visible || el.esModo ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400 line-through'}`}>{esFiscal && el.labelFiscal ? el.labelFiscal : el.label}</span>
                        {!el.esModo && (() => {
                          ...
                          return (
                            <div className="flex flex-col items-end gap-0.5 shrink-0">
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => cambiar(-1)} className="w-6 h-6 rounded-md bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-gray-300 flex items-center justify-center hover:bg-gray-300"><Icon icon="solar:minus-square-bold" width={14} /></button>
                                ...
                                <button onClick={() => cambiar(1)} className="w-6 h-6 rounded-md bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-gray-300 flex items-center justify-center hover:bg-gray-300"><Icon icon="solar:add-square-bold" width={14} /></button>
                              </div>
                              {esOverride && (
                                <span className={`text-[10px] leading-none ${propio !== undefined ? 'text-violet-500' : 'text-gray-400'}`}>
