...
export const useFacturacionViewModel = () => {
    ...
    // buscaba una boleta y lo dejaba como estaba, la nota salía con serie FCA1
    // (serie de nota sobre factura) apuntando a una boleta. SUNAT rechaza esa
    // contradicción con el error 2116 y la anulación nunca ocurre, aunque el
    ...
    const handleProductClick = (product: any) => {
        ...
        const precioUnitarioPaquete = esPaqueteConPrecio
            ? (Number(product.precioPaquete) / unidadesPorPaquete) * factorNative
            : null;
    ...
    const handleSaveQuotationConfig = (config: QuotationConfig) => {
        ...
        if (Object.keys(defaultsPayload).length > 0) {
            useEmpresasStore.getState().actualizarMiEmpresa(defaultsPayload as any)
                .then(() => {
                    // Refresca auth.empresa para que el default aplique de inmediato
                    // en la siguiente cotización de esta misma sesión.
    ...
    const handleChangeQuotationCurrency = (next: string) => {
        ...
        if ((targetUSD || hayUSDOrigen) && (!Number.isFinite(tc) || tc <= 0)) {
            ...
            // moneda se quedaba en Soles sin avisar de forma persistente, y el ítem
            // recién editado seguía mostrando "S/" aunque el usuario ya había elegido
            // Dólares. Se guarda la intención y se reintenta sola en cuanto llegue el TC.
    ...
    const handleCrearClienteCelular = async (celularRaw: string): Promise<boolean> => {
        ...
        try {
            const payload = {
                nombre: '',
                tipoDoc: 'OTRO',
                // El DTO exige nroDoc no vacío: "0" = sin documento (igual que ModalClient).
                nroDoc: '0',
                telefono: celular,
                persona: 'CLIENTE',
                direccion: '',
