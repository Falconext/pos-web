        ...
        >
            <div className="px-4 pt-3 pb-4 space-y-3">
                ...
                <div className="rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                    <div className="flex items-center gap-3 px-3 py-2 bg-gray-50 dark:bg-slate-800/60 text-[11px] font-bold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                        <input type="checkbox" checked={todosMarcados} onChange={toggleTodos} className="h-3.5 w-3.5 rounded border-gray-300 text-violet-600 focus:ring-violet-500" aria-label="Seleccionar todos" />
                        <span className="flex-1">Producto</span>
                        <span className="w-24 text-right">Stock aquí</span>
                        <span className="w-28 text-right">Estado</span>
                    </div>
                    <div className="max-h-[52vh] overflow-y-auto divide-y divide-gray-100 dark:divide-slate-800">
                        {loading && (
                            <div className="px-3 py-6 text-center text-xs text-gray-400"><Icon icon="svg-spinners:ring-resize" className="inline mr-1" /> Cargando…</div>
                        )}
                        {!loading && visibles.length === 0 && (
                            <div className="px-3 py-6 text-center text-xs text-gray-400">Sin productos para mostrar.</div>
                        )}
                        {!loading && visibles.map(r => {
                            const disponible = r.disponibleEnSede !== false;
                            const stock = Number(r.stockBase ?? r.stock ?? 0);
                            return (
                                <label key={r.id} className="flex items-center gap-3 px-3 py-2 text-xs cursor-pointer hover:bg-violet-50/40 dark:hover:bg-violet-900/10">
                                    <input type="checkbox" checked={seleccion.has(r.id)} onChange={() => toggleUno(r.id)} className="h-3.5 w-3.5 rounded border-gray-300 text-violet-600 focus:ring-violet-500" />
                                    <div className="flex-1 min-w-0">
                                        <p className="font-semibold text-gray-800 dark:text-gray-100 truncate">{r.descripcion}</p>
                                        <p className="text-[10px] text-gray-400">{r.codigo}</p>
                                    </div>
                                    <span className="w-24 text-right font-semibold text-gray-700 dark:text-gray-200">{stock}</span>
                                    <span className="w-28 text-right">
                                        {disponible ? (
                                            ...
                                            <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold text-gray-500 dark:bg-slate-700 dark:text-gray-300"><Icon icon="mdi:eye-off-outline" width={11} /> No asignado</span>
                                        )}
                                    </span>
                                </label>
                            );
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2 text-[11px] text-gray-500 dark:text-gray-400">
                        <button type="button" disabled={page <= 1 || loading} onClick={() => setPage(p => Math.max(1, p - 1))} className="rounded-lg border border-gray-200 dark:border-slate-700 px-2 py-1 disabled:opacity-40"><Icon icon="mdi:chevron-left" /></button>
                        Página {page} de {totalPages}
                        <button type="button" disabled={page >= totalPages || loading} onClick={() => setPage(p => Math.min(totalPages, p + 1))} className="rounded-lg border border-gray-200 dark:border-slate-700 px-2 py-1 disabled:opacity-40"><Icon icon="mdi:chevron-right" /></button>
                    </div>
