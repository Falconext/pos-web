...
export const ProductVariantsManager: React.FC<{ vm: ViewProps }> = ({ vm }) => {
  ...
  // Lightbox de sugerencias por color: { color, index } o null.
  const [colorLightbox, setColorLightbox] = useState<{ color: string; index: number } | null>(null);
  ...
  const handleColorImage = (color: string, file?: File | null) => {
    ...
    // El archivo manual reemplaza a cualquier URL sugerida previamente elegida.
    setVariantImageUrls((prev: Record<string, string>) => {
      const next = { ...prev };
      delete next[color];
      return next;
    });
  ...
  const removeColorImage = (color: string) => {
    ...
    setVariantImageFiles(nextFiles);
    setVariantImagePreviews(nextPreviews);
    // Limpia también la URL sugerida elegida para el color.
    setVariantImageUrls((prev: Record<string, string>) => {
      const next = { ...prev };
      delete next[color];
      return next;
  ...
  // Elige una imagen sugerida (IA/Serper) para un color: la marca en el vm y además
  // la escribe como URL externa en las filas del color, de modo que persista aunque
  // la copia a S3 falle (mismo respaldo hotlink que usa la imagen principal).
  const pickColorSuggestion = (color: string, url: string) => {
    selectColorImageCandidate(color, url);
    const nextConfig = buildConfig(opcionesAtributos).map((row) =>
      row.valoresAtributos?.[colorOptionName] === color
        ? { ...row, imagenUrl: url, imagenUrlDisplay: url }
        : row,
    );
    setFormValues({ ...formValues, variantesConfig: nextConfig } as any);
  };

  // ── Galería del color desde sugerencias (multi-selección de URLs) ──
  const isUrlInColorGallery = (color: string, url: string) =>
    (variantGalleryUrls?.[color] || []).includes(url);

  const toggleColorGalleryUrl = (color: string, url: string) => {
    setVariantGalleryUrls((prev: Record<string, string[]>) => {
      const list = prev[color] || [];
      const next = list.includes(url) ? list.filter((u) => u !== url) : [...list, url];
      const copy = { ...prev };
      if (next.length) copy[color] = next;
      else delete copy[color];
      return copy;
    });
  };

  // ── Lightbox (vista grande de sugerencias con acciones) ──
  const openColorLightbox = (color: string, index: number) => setColorLightbox({ color, index });
  const closeColorLightbox = () => setColorLightbox(null);
  const moveLightbox = (delta: number) => {
    setColorLightbox((prev) => {
      if (!prev) return prev;
      const list = variantImageCandidates?.[prev.color] || [];
      if (!list.length) return prev;
      const nextIndex = (prev.index + delta + list.length) % list.length;
  ...
  return (
    <div className="col-span-1 md:col-span-2 mt-4 rounded-3xl border border-violet-100 bg-gradient-to-br from-violet-50/80 to-white p-5 dark:border-violet-900/40 dark:from-violet-950/20 dark:to-slate-950/10">
        ...
        <div className="mt-5 rounded-2xl border border-white/70 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-950/20">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
            {colorValues.map((color) => {
              ...
              const pendingGalleryUrls = variantGalleryUrls[color] || [];
              return (
                <div key={color} className="rounded-2xl border border-gray-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900">
                  ...
                  <label className="flex h-36 cursor-pointer items-center justify-center overflow-hidden rounded-xl border border-dashed border-gray-300 bg-white p-1 text-center text-[11px] font-bold text-gray-500 transition hover:border-violet-400 dark:border-slate-700 dark:bg-slate-800">
                    {preview ? (
                      <img src={preview} alt={color} className="h-full w-full object-contain" />
                    ) : (
                      <span>
                  ...
                  {productSections?.autoGenerarImagen && (
                    <>
                      <button
                        type="button"
                        onClick={() => handleAutoImageColor(color)}
                        disabled={Boolean(variantImageCandidatesLoading?.[color]) || !formValues.descripcion}
                        className={`mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg px-2 py-1.5 text-[11px] font-bold transition ${
                          variantImageCandidatesLoading?.[color] || !formValues.descripcion
                        ...
                        title={`Buscar imágenes de "${color}"`}
                      >
                        <Icon
                          icon={variantImageCandidatesLoading?.[color] ? 'line-md:loading-loop' : 'solar:magic-stick-3-bold-duotone'}
                          width={15}
                        />
                        {variantImageCandidatesLoading?.[color] ? 'Buscando...' : 'Auto-Generar'}
                      </button>
                      {(variantImageCandidates?.[color]?.length ?? 0) > 0 && (
                        <div className="mt-2 rounded-lg border border-indigo-100 bg-indigo-50/40 p-2 dark:border-indigo-900/40 dark:bg-indigo-950/20">
                          <div className="mb-1.5 flex items-center justify-between">
                            <span className="text-[10px] font-black uppercase tracking-wide text-indigo-700 dark:text-indigo-300">
                              Sugeridas · {color}
                            </span>
                            <button
                              type="button"
                              className="text-[10px] text-gray-500 underline hover:text-gray-700 dark:text-gray-400"
                              onClick={() =>
                                setVariantImageCandidates((prev: Record<string, string[]>) => {
                                  const next = { ...prev };
                                  delete next[color];
                                  return next;
                                })
                          ...
                          <p className="mb-1.5 text-[9px] font-semibold text-indigo-500/80 dark:text-indigo-300/70">
                            Clic para ver en grande y elegir principal o galería.
                          </p>
                          <div className="grid grid-cols-3 gap-1.5">
                            {variantImageCandidates[color].map((url: string, sIdx: number) => {
                              const isSelected = variantImageUrls?.[color] === url;
                              const inGallery = isUrlInColorGallery(color, url);
                              return (
                                <button
                                  key={url}
                                  type="button"
                                  onClick={() => openColorLightbox(color, sIdx)}
                                  className={`relative h-14 overflow-hidden rounded-lg border-2 bg-white transition-all ${
                                    isSelected
                                      ? 'border-indigo-500 ring-2 ring-indigo-200 dark:ring-indigo-900'
                                      : 'border-transparent hover:border-indigo-300'
                                  }`}
                                >
                                  <img src={url} alt={`${color} sugerida`} className="h-full w-full object-contain" />
                                  {isSelected && (
                                    <span className="absolute left-0.5 top-0.5 rounded bg-indigo-600 px-1 py-0.5 text-[8px] font-bold text-white" title="Imagen principal del color">
                                      ★
                                    </span>
                                  )}
                                  {inGallery && (
                                    <span className="absolute right-0.5 top-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-violet-600 text-[8px] font-bold text-white" title="En la galería del color">
                                      <Icon icon="solar:gallery-bold" width={9} />
                                    </span>
                                  )}
                                </button>
                              );
                  ...
                  <div className="mt-3 rounded-xl border border-gray-100 bg-gray-50 p-2 dark:border-slate-800 dark:bg-slate-950/30">
                    ...
                    {savedGallery.length === 0 && pendingGallery.length === 0 && pendingGalleryUrls.length === 0 ? (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {pendingGalleryUrls.map((url) => (
                          <div key={`url-${url}`} className="group relative h-16 overflow-hidden rounded-lg bg-white ring-2 ring-indigo-300">
                            <img src={url} alt={`${color} sugerida galería`} className="h-full w-full object-contain" />
                            <button
                              type="button"
                              onClick={() => toggleColorGalleryUrl(color, url)}
      ...
      {colorLightbox && (() => {
        const list = variantImageCandidates[colorLightbox.color] || [];
        const url = list[colorLightbox.index];
        if (!url) return null;
        const isPrincipal = variantImageUrls?.[colorLightbox.color] === url;
        const inGallery = isUrlInColorGallery(colorLightbox.color, url);
        return (
          <div
            className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4"
            onClick={closeColorLightbox}
          >
            <div
              className="relative w-full max-w-2xl rounded-2xl bg-white p-4 shadow-2xl dark:bg-slate-900"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="mb-3 flex items-center justify-between">
                <span className="text-sm font-black text-gray-800 dark:text-white">
                  Sugeridas · {colorLightbox.color}
                  <span className="ml-2 text-xs font-semibold text-gray-400">
                    {colorLightbox.index + 1}/{list.length}
                  </span>
                </span>
                <button type="button" onClick={closeColorLightbox} className="text-gray-400 transition hover:text-gray-600 dark:hover:text-gray-200">
                  <Icon icon="solar:close-circle-bold" width={26} />
                </button>
              </div>
              <div className="relative flex h-[55vh] items-center justify-center overflow-hidden rounded-xl bg-gray-50 dark:bg-slate-800">
                {list.length > 1 && (
                  <button
                    type="button"
                    onClick={() => moveLightbox(-1)}
                    className="absolute left-2 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow transition hover:bg-white dark:bg-slate-900/80 dark:text-gray-200"
                    title="Anterior"
                  >
                    <Icon icon="solar:alt-arrow-left-bold" width={22} />
                  </button>
                )}
                <img src={url} alt={colorLightbox.color} className="max-h-full max-w-full object-contain" />
                {list.length > 1 && (
                  <button
                    type="button"
                    onClick={() => moveLightbox(1)}
                    className="absolute right-2 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow transition hover:bg-white dark:bg-slate-900/80 dark:text-gray-200"
                    title="Siguiente"
                  >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => pickColorSuggestion(colorLightbox.color, url)}
                ...
                >
                  ...
                  {isPrincipal ? 'Es la principal' : 'Usar como principal'}
                </button>
                <button
                  type="button"
                  onClick={() => toggleColorGalleryUrl(colorLightbox.color, url)}
                  className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-bold transition ${
                    inGallery
                      ? 'bg-violet-600 text-white'
                      : 'bg-violet-50 text-violet-700 hover:bg-violet-100 dark:bg-violet-900/40 dark:text-violet-300'
                  }`}
                >
                  <Icon icon={inGallery ? 'solar:gallery-check-bold' : 'solar:gallery-add-bold'} width={16} />
                  {inGallery ? 'Quitar de galería' : 'Agregar a galería'}
                </button>
                <span className="ml-auto self-center text-[11px] font-semibold text-gray-400">
                  Puedes agregar varias a la galería
                </span>
              </div>
            </div>
          </div>
