...
import { MonoAreaChart, MonoBarChart, MonoDonutChart, MONO_SERIES, MONO_SEGMENTS, ACCENT, fmtInt } from '@/components/charts/mono';
...
// Mini-stat para la sección de proyección
const ProyStat = ({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: boolean }) => (
    <div className={`rounded-xl border p-3 ${accent ? 'border-violet-200 bg-violet-50/60 dark:border-violet-500/25 dark:bg-violet-500/10' : 'border-gray-100 bg-gray-50/60 dark:border-slate-800 dark:bg-slate-800/40'}`}>
        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 truncate">{label}</p>
        <p className={`mt-1 text-lg font-black tracking-tight truncate ${accent ? 'text-violet-600 dark:text-violet-400' : 'text-gray-900 dark:text-white'}`}>{value}</p>
        {sub && <p className="text-[10px] font-medium text-gray-400 truncate">{sub}</p>}
    </div>
);
...
function TabDashboard({ vm }: { vm: ReturnType<typeof useSistemaFinanzasViewModel> }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid lg:grid-cols-3 gap-5">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-[#111827] rounded-2xl border border-gray-100 dark:border-slate-800 p-5 shadow-sm">
                    ...
                    <div className="flex items-center gap-4 mb-3">
                        <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: MONO_SERIES[0] }} /><span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Ingresos</span></span>
                        <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: MONO_SERIES[1] }} /><span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Gastos</span></span>
                    </div>
                    <MonoAreaChart
                        data={vm.tendencia}
                        index="mes"
                            ...
                            <div className="space-y-1.5 mt-2">
                                {pieData.map((p: any, i: number) => (
                                    <div key={p.name} className="flex items-center gap-2">
                                        <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: MONO_SEGMENTS[i % MONO_SEGMENTS.length] }} />
            ...
            {vm.proyeccion && (
                <div className="relative overflow-hidden bg-white dark:bg-[#111827] rounded-2xl border border-violet-100 dark:border-violet-500/20 p-5 shadow-sm">
                    <div className="pointer-events-none absolute -right-16 -top-16 h-44 w-44 rounded-full opacity-[0.07] blur-2xl" style={{ background: 'radial-gradient(circle, #7551FF 0%, transparent 70%)' }} />
                    <div className="relative flex items-start justify-between gap-3 mb-4 flex-wrap">
                        <div>
                            <h3 className="text-sm font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                <Icon icon="solar:graph-new-up-bold-duotone" className="text-violet-500 text-lg" />
                                Proyección de ingreso recurrente
                            </h3>
                            <p className="text-[11px] text-gray-400 mt-0.5">
                                Estimación a {vm.proyeccion.mesesFuturos} meses según el crecimiento neto histórico (altas − bajas). Es una proyección, no dinero cobrado.
                            </p>
                        </div>
                        <div className="flex gap-1">
                            {[1, 3, 6, 12].map((m) => (
                                <button
                                    key={m}
                                    onClick={() => vm.setMesesProyeccion(m)}
                                    className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors ${vm.mesesProyeccion === m ? 'bg-violet-600 text-white' : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700'}`}
                                >
                                    {m}M
                                </button>
                            ))}
                    ...
                    <div className="relative grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
                        <ProyStat label="MRR actual" value={fmt(vm.proyeccion.mrrActual)} sub={`${vm.proyeccion.empresasActivas} activas`} />
                        <ProyStat label={`MRR proyectado (${vm.proyeccion.mesesFuturos}m)`} value={fmt(vm.proyeccion.mrrProyectadoFinal)} accent />
                        <ProyStat label={`Ganancia acumulada (${vm.proyeccion.mesesFuturos}m)`} value={fmt(vm.proyeccion.gananciaAcumulada)} accent />
                        <ProyStat label="Altas prom./mes" value={`+${vm.proyeccion.avgNuevos}`} sub={`ticket ${fmt(vm.proyeccion.ticketPromedio)}`} />
                        <ProyStat label="Churn mensual" value={`${vm.proyeccion.churnRate}%`} sub={`${vm.proyeccion.avgBajas} bajas/mes`} />
                    </div>

                    <div className="relative">
                        <MonoAreaChart
                            data={vm.proyeccion.serie}
                            index="mes"
                            categories={['mrrProyectado']}
                            valueFormatter={(v) => fmt(v)}
                            height={260}
                        />
                    </div>
...
function TabMovimientos({ vm }: { vm: ReturnType<typeof useSistemaFinanzasViewModel> }) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* ── Filtros compartidos ── */}
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-white dark:bg-[#111827] rounded-2xl border border-gray-100 dark:border-slate-800 px-4 py-3 shadow-sm">
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                    ...
                    <button
                        ...
                        className="p-2 rounded-xl bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-500 transition-colors"
                        title="Actualizar"
                    >
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-md shadow-emerald-200 transition-colors"
                    >
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-sm font-bold shadow-md shadow-violet-200 transition-colors"
                    >
                        <Icon icon="solar:add-circle-bold" width={18} />
                        Registrar gasto
                    </button>
            ...
            {/* ── Tabla Ingresos ── */}
            <div className="bg-white dark:bg-[#111827] rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm">
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-slate-800 bg-emerald-50/50 dark:bg-emerald-900/10">
                    <h3 className="text-sm font-bold text-emerald-700 dark:text-emerald-400 flex items-center gap-2">
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs font-black">{vm.ingresos.length}</span>
                    </h3>
                    <Select
                        label=""
                        name="filtroTipo"
                ...
                {vm.loadingIngresos ? (
                    <div className="flex items-center justify-center h-32"><Icon icon="eos-icons:loading" className="w-8 h-8 text-emerald-500" /></div>
                ) : vm.ingresos.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 text-gray-400">
                        <Icon icon="solar:wallet-money-bold-duotone" width={40} className="mb-2 opacity-30" />
                                ...
                                {vm.ingresos.map((ing) => {
                                    ...
                                    return (
                                        <tr key={ing.id} className="hover:bg-gray-50/60 dark:hover:bg-slate-800/40 transition-colors">
                                            <td className="px-5 py-3.5 text-gray-600 dark:text-gray-400 whitespace-nowrap">{new Date(ing.fecha).toLocaleDateString('es-PE')}</td>
                                            ...
                                            <td className="px-5 py-3.5 text-right font-bold text-emerald-600 dark:text-emerald-400 whitespace-nowrap">{fmt(ing.monto)}</td>
                                            <td className="px-5 py-3.5">
                                                <div className="flex items-center gap-1 justify-end">
                                                    <button onClick={() => vm.abrirEditarIngreso(ing)} className="p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-500 transition-colors"><Icon icon="solar:pen-bold" width={15} /></button>
                                                    <button onClick={() => vm.eliminarIngreso(ing.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500 transition-colors"><Icon icon="solar:trash-bin-trash-bold" width={15} /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
            ...
            {/* ── Tabla Gastos ── */}
            <div className="bg-white dark:bg-[#111827] rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm">
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-slate-800 bg-violet-50/50 dark:bg-violet-900/10">
                    <h3 className="text-sm font-bold text-violet-700 dark:text-violet-400 flex items-center gap-2">
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-400 text-xs font-black">{vm.gastos.length}</span>
                    </h3>
                    <Select
                        label=""
                ...
                {vm.loadingGastos ? (
                    <div className="flex items-center justify-center h-32"><Icon icon="eos-icons:loading" className="w-8 h-8 text-violet-500" /></div>
                ) : vm.gastos.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 text-gray-400">
                        <Icon icon="solar:card-search-bold-duotone" width={40} className="mb-2 opacity-30" />
                                ...
                                {vm.gastos.map((g) => {
                                    ...
                                    return (
                                        <tr key={g.id} className="hover:bg-gray-50/60 dark:hover:bg-slate-800/40 transition-colors">
                                            <td className="px-5 py-3.5 text-gray-600 dark:text-gray-400 whitespace-nowrap">{new Date(g.fecha).toLocaleDateString('es-PE')}</td>
                                            <td className="px-5 py-3.5">
                                            ...
                                            <td className="px-5 py-3.5 text-right font-bold text-red-600 dark:text-red-400 whitespace-nowrap">{fmt(g.monto)}</td>
                                            ...
                                            <td className="px-5 py-3.5">
                                                <div className="flex items-center gap-1 justify-end">
                                                    <button onClick={() => vm.abrirEditar(g)} className="p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-500 transition-colors"><Icon icon="solar:pen-bold" width={15} /></button>
                                                    <button onClick={() => vm.eliminarGasto(g.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500 transition-colors"><Icon icon="solar:trash-bin-trash-bold" width={15} /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-[#111827] rounded-2xl border border-gray-100 dark:border-slate-800 p-5 shadow-sm">
                ...
                <div className="flex items-center gap-4 mb-3">
                    <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: MONO_SERIES[0] }} /><span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Nuevas empresas</span></span>
                    <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: MONO_SERIES[1] }} /><span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Bajas</span></span>
                </div>
                <MonoBarChart
                    data={vm.tendencia}
                    index="mes"
