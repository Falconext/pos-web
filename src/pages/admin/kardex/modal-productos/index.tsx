...
const ModalProduct = ({ setSelectProduct, isInvoice, initialForm, formValues, setErrors, isOpenModal, setFormValues, closeModal, isEdit, errors, setIsOpenModal }: IPropsProducts) => {
    ...
    return (
        <>
            {isOpenModal && <Modal width={isRestaurante ? "900px" : isFarmacia ? "1000px" : "1400px"} isOpenModal={isOpenModal} closeModal={closeModal} title={`${isEdit ? `Editar ${labels.titulo}` : `Nuevo ${labels.titulo}`}${isRestaurante || isFarmacia ? '' : ' - Gestión Completa'}`}>
                <div className={`${isRestaurante ? 'grid-cols-1 md:grid-cols-2' : isFarmacia ? 'grid-cols-1 md:grid-cols-3' : 'grid-cols-3'} grid px-4 gap-5`}>
                    <div className={isFarmacia ? 'md:col-span-1' : ''}>
                        ...
                        {/* Análisis Financiero en Tiempo Real - Solo para rubros genéricos (no restaurante ni farmacia) */}
                        ...
                        <div className="col-span-2 px-3 mt-3">
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-4">
                                ...
                                <Select defaultValue={formValues?.unidadMedidaNombre} error={""} isSearch options={unitOfMeasure?.map((item: ICategory) => ({
                                    id: item?.id,
                                    value: `${item?.nombre}`
                                }))} id="unidadMedidaId" name="unidadMedidaNombre" value="" onChange={handleChangeSelect} icon="clarity:box-plot-line" isIcon label="Unidad de medida" withLabel />
                            </div>
                            ...
                            <div className="p-4 rounded-lg border border-blue-100 bg-blue-50/30 mb-4">
                                <h5 className="text-sm font-semibold text-blue-700 mb-3 flex items-center gap-2">
                                    <Icon icon="solar:pill-bold-duotone" width={16} />
                                    Información del Medicamento
                                </h5>
                                <div className="grid grid-cols-4 gap-3">
                                    <InputPro autocomplete="off" value={(formValues as any)?.principioActivo || ''} name="principioActivo" onChange={handleChange} isLabel label="Principio Activo" placeholder="Paracetamol" />
                                    <InputPro autocomplete="off" value={(formValues as any)?.concentracion || ''} name="concentracion" onChange={handleChange} isLabel label="Concentración" placeholder="500 mg" />
                                    <InputPro autocomplete="off" value={(formValues as any)?.presentacion || ''} name="presentacion" onChange={handleChange} isLabel label="Presentación" placeholder="Caja x 100" />
                                    <InputPro autocomplete="off" value={(formValues as any)?.laboratorio || ''} name="laboratorio" onChange={handleChange} isLabel label="Laboratorio" placeholder="Bayer" />
                                </div>
                            </div>

                            {/* Unidades de Compra/Venta */}
                            <div className="p-4 rounded-lg border border-green-100 bg-green-50/30 mb-4">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    <InputPro autocomplete="off" type="number" value={(formValues as any)?.factorConversion || 1} name="factorConversion" onChange={handleChange} isLabel label="Factor Conversión" placeholder="10" />
                                </div>
                                    ...
                                    {isEdit ? (
                                        <div className="space-y-3">
                                                ...
                                                <div className="bg-green-50 p-2 rounded-lg text-center">
                                                    <div className="text-lg font-bold text-green-600">
                                                        {tipoAjusteStock === 'ninguno' ? stockOriginal :
                                                            tipoAjusteStock === 'reemplazar' ? cantidadAjuste :
                                                                tipoAjusteStock === 'sumar' ? stockOriginal + cantidadAjuste :
                                                                    Math.max(0, stockOriginal - cantidadAjuste)}
                                                    </div>
                                                    <div className="text-xs text-gray-500">Resultante</div>
                                                </div>
                                            </div>
                                            <div className="flex flex-wrap gap-1">
                                                {[
                                                    ...
                                                    <button
                                                        ...
                                                        className={`px-2 py-1 rounded text-xs font-medium transition-all ${tipoAjusteStock === tipo.value
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mt-2">
                                                <InputPro autocomplete="off" type="number" value={formValues?.stockMinimo ?? ''} name="stockMinimo" onChange={handleChange} isLabel label="Stock mín." placeholder="5" />
                                                <InputPro autocomplete="off" type="number" value={formValues?.stockMaximo ?? ''} name="stockMaximo" onChange={handleChange} isLabel label="Stock máx." placeholder="100" />
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-2 gap-3">
                                            <InputPro autocomplete="off" type="number" value={formValues?.stock} error={errors.stock} name="stock" onChange={handleChange} isLabel label="Stock Inicial" placeholder="100" />
                                            <InputPro autocomplete="off" type="number" value={formValues?.stockMinimo ?? ''} name="stockMinimo" onChange={handleChange} isLabel label="Stock mínimo" placeholder="10" />
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                <div className="flex gap-5 justify-end mt-5 mb-5 md:pr-5">
                    ...
                    <Button color="secondary" onClick={handleSubmitProduct} disabled={loading}>
                        {loading ? (
