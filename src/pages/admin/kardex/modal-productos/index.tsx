    ...
    const handleSubmitProduct = async () => {
        ...
        if (Number(formValues?.productoId) !== 0 && isEdit) {
            ...
            try {
                ...
                } else if (autoGeneratedImageUrl) {
                    // Auto-generated external URL - upload to S3
                    try {
                        const resp = await apiClient.post(`/producto/${formValues.productoId}/imagen-url`, { url: autoGeneratedImageUrl });
                        const signed = resp?.data?.signedUrl || resp?.data?.data?.signedUrl;
