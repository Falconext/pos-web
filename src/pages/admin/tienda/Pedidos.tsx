        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 mt-6">
          {vm.estadisticas.map((stat) => (
            <button key={stat.value} onClick={() => vm.setFiltroEstado(vm.filtroEstado === stat.value ? '' : stat.value)}
              className={`p-4 rounded-xl border-2 transition-all hover:shadow-md ${vm.filtroEstado === stat.value ? `${stat.color} shadow-sm scale-105` : 'bg-white border-gray-200 hover:border-gray-300'}`}>
          ...
          {vm.pedidosFiltrados.map((pedido) => {
            ...
            return (
              <div key={pedido.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all overflow-hidden">
                <div className="p-5 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => vm.toggleOrderExpanded(pedido.id)}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${pedido.tipoEntrega === 'ENVIO' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-sky-50 text-sky-700 border border-sky-200'}`}>
                    ...
                    <div className="flex items-center gap-4 flex-shrink-0">
                      ...
                      <Icon icon={isExpanded ? "solar:alt-arrow-up-bold" : "solar:alt-arrow-down-bold"} className="w-5 h-5 text-gray-400" />
                    </div>
                  </div>
                </div>
                {isExpanded && (
                    ...
                    <div className="p-5 space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="bg-white rounded-xl p-4 border border-gray-200">
                          <div className="flex items-center gap-2 mb-3"><Icon icon="solar:user-bold-duotone" className="w-5 h-5 text-blue-600" /><h4 className="font-semibold text-gray-900">Cliente</h4></div>
                          <div className="space-y-2 text-sm">
                            <p className="font-medium text-gray-900">{pedido.clienteNombre}</p>
                            <p className="flex items-center gap-2 text-gray-600"><Icon icon="solar:phone-bold-duotone" className="w-4 h-4" />{pedido.clienteTelefono}</p>
                            {pedido.clienteEmail && <p className="flex items-center gap-2 text-gray-600"><Icon icon="solar:letter-bold-duotone" className="w-4 h-4" />{pedido.clienteEmail}</p>}
                          </div>
                        </div>
                        <div className="bg-white rounded-xl p-4 border border-gray-200">
                          <div className="flex items-center gap-2 mb-3"><Icon icon="solar:map-point-bold-duotone" className="w-5 h-5 text-green-600" /><h4 className="font-semibold text-gray-900">Entrega</h4></div>
                          <div className="space-y-2 text-sm text-gray-600">
                            {pedido.clienteDireccion && <p>{pedido.clienteDireccion}</p>}
                            {pedido.clienteReferencia && <p className="text-gray-500 italic">{pedido.clienteReferencia}</p>}
                          </div>
                        </div>
                      </div>
                      <div className="bg-white rounded-xl p-4 border border-gray-200">
                        <div className="flex items-center gap-2 mb-3"><Icon icon="solar:bag-4-bold-duotone" className="w-5 h-5 text-purple-600" /><h4 className="font-semibold text-gray-900">Productos</h4></div>
                        <div className="space-y-2">
                          {pedido?.items?.map((item: any) => (
                            <div key={item.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                              <div className="flex items-center gap-3 flex-1">
                                <span className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-sm font-bold text-gray-700">{item.cantidad}</span>
