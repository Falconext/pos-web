  ...
  return (
    <div className="min-h-screen px-2 pb-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        <div><h1 className="text-2xl font-bold text-gray-900 tracking-tight">Empresas</h1><p className="text-sm text-gray-500 mt-1">Administra las empresas registradas en el sistema</p></div>
        <Button variant="default" onClick={() => { vm.setEmpresaEditingId(undefined); vm.setEmpresaModalMode('create'); vm.setOpenEmpresaModal(true); }} className="flex items-center gap-2">
          <Icon icon="solar:add-circle-bold" className="text-lg" />Nueva Empresa
        </Button>
      </div>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {vm.error && <div className="mx-5 mt-4 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg flex items-center gap-2"><Icon icon="solar:danger-circle-bold" className="text-xl" />{vm.error}</div>}
        <div className="p-4">
          {vm.empresasTable?.length > 0 ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">{vm.searchTerm ? 'Intenta con otros términos de búsqueda' : 'Aún no tienes empresas registradas'}</p>
