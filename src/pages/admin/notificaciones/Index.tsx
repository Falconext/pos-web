    ...
    <div className="min-h-screen px-2 pb-4">
      ...
      {vm.notificaciones?.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gray-100 mb-4"><Icon icon="solar:bell-off-linear" className="h-10 w-10 text-gray-400" /></div>
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No tienes notificaciones</h3>
          <p className="text-gray-600 max-w-md mx-auto">Cuando recibas notificaciones importantes, aparecerán aquí.</p>
        </div>
      ) : (
              ...
              <div key={notificacion.id} onClick={() => vm.handleNotificacionClick(notificacion)}
                ...
                <div className="p-4 pb-3 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="flex-shrink-0 w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center"><Icon icon={icon} className={`h-5 w-5 ${iconColor}`} /></div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-gray-900 text-sm line-clamp-2 leading-tight">{notificacion.titulo}</h3>
                      {!notificacion.leida && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500 text-white mt-1">Nueva</span>}
                    </div>
                  </div>
                </div>
                <div className="p-4">
                  <p className="text-sm text-gray-700 mb-3 line-clamp-3">{notificacion.mensaje}</p>
                  <div className="flex items-center gap-1 text-xs text-gray-500"><Icon icon="solar:clock-circle-bold-duotone" className="h-3.5 w-3.5" /><span>{vm.formatFecha(notificacion.creadoEn)}</span></div>
                </div>
              </div>
            );
          })}
