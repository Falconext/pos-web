...
function SectionTitle({ children, cp }: { children: React.ReactNode; cp: string }) {
  return (
    <div className="mb-5 flex min-w-0 items-center gap-3">
      ...
      <h2 className="min-w-0 text-xs font-black uppercase tracking-widest text-gray-400">{children}</h2>
    </div>
  );
}
...
export default function MayeCheckout({
  ...
  return (
    <motion.div
      className="min-h-screen overflow-x-hidden bg-[#F8F9FA]"
    ...
    >
      ...
      <motion.main className="mx-auto max-w-screen-xl px-4 pb-20 pt-6 md:px-8 md:pt-8" variants={mayeStagger}>

        {/* ── Top bar ── */}
        <motion.div className="mb-6 flex min-w-0 items-center justify-between md:mb-8" variants={mayeCard}>
          <motion.button
        ...
        <div className="mb-5 flex min-w-0 items-center justify-between gap-1 sm:hidden">
        ...
        <motion.div className="flex flex-col items-stretch gap-6 lg:flex-row lg:items-start" variants={mayeSection}>
          <div className="flex-1 min-w-0 space-y-5">
            <motion.div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm" variants={mayeCard}>
              <div className="flex items-center justify-between border-b border-gray-100 px-4 py-4 sm:px-6">
                <h2 className="text-lg font-black text-gray-900 sm:text-base">
                  Mi carrito{' '}
              ...
              <motion.div className="divide-y divide-gray-100" variants={mayeStagger}>
                {carrito.map(item => {
                  ...
                  return (
                    <motion.div
                      key={item.id}
                      className="grid grid-cols-[76px_minmax(0,1fr)] gap-3 px-4 py-4 sm:flex sm:items-center sm:gap-4 sm:px-5"
                      variants={mayeCard}
                      layout
                    >
                      <div className="h-[76px] w-[76px] flex-shrink-0 overflow-hidden rounded-2xl border border-gray-100 bg-gray-50 p-1 sm:h-20 sm:w-20 sm:rounded-xl">
                        {item.imagenUrl
                      ...
                      <div className="min-w-0 flex-1 self-center">
                        <p className="line-clamp-3 break-words text-sm font-black leading-snug text-gray-900 sm:line-clamp-2">{item.descripcion}</p>
                        {item.partNumber && (
                          <p className="text-[10px] text-gray-400 font-mono mt-0.5">PN: {item.partNumber}</p>
                        )}
                        <button
                          onClick={() => removeItem(item.id)}
                          className="flex items-center gap-1 text-xs text-gray-500 transition-colors mt-1.5"
                      ...
                      <div className="col-span-2 flex min-w-0 items-center justify-between gap-3 rounded-2xl border border-gray-100 bg-gray-50 px-3 py-2 sm:col-span-1 sm:flex-shrink-0 sm:border-gray-200">
                        <div className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-2 py-1.5">
                          <button
                            onClick={() => updateQuantity(item.id, item.cantidad - 1)}
                            className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-900"
                        ...
                        <div className="min-w-0 text-right">
                          <p className="whitespace-nowrap text-base font-black" style={{ color: hasDiscount ? cp : '#111827' }}>
            ...
            {configEnvio && (configEnvio.aceptaEnvio || configEnvio.aceptaRecojo) && (
              <motion.div className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6" variants={mayeCard} initial="initial" whileInView="animate" viewport={mayeViewport}>
                <SectionTitle cp={cp}>Método de entrega</SectionTitle>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {configEnvio.aceptaEnvio && (
                    <label
                      className="flex min-w-0 cursor-pointer items-center gap-3 rounded-2xl border-2 px-4 py-3 text-sm font-bold transition-all"
                      style={
                    ...
                    >
                      ...
                      <Icon icon="solar:delivery-bold" width={20} className="flex-shrink-0" />
                      <span className="min-w-0 flex-1">
                        <span className="block leading-tight">Delivery</span>
                        {configEnvio.costoEnvio > 0 && (
                          <span className="mt-0.5 block text-xs font-bold opacity-70">S/ {Number(configEnvio.costoEnvio).toFixed(2)}</span>
                        )}
                      </span>
                    </label>
                  )}
                  {configEnvio.aceptaRecojo && (
                    ...
                    >
                      ...
                      <Icon icon="solar:shop-bold" width={20} className="flex-shrink-0" />
                      <span className="min-w-0 leading-tight">Recojo en tienda</span>
                    </label>
                  )}
                </div>

                {formData.tipoEntrega === 'RECOJO' && configEnvio.direccionRecojo && (
                  <div className="mt-4 flex min-w-0 items-start gap-3 rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3">
                    <Icon icon="solar:map-point-bold" width={18} style={{ color: cp }} className="flex-shrink-0" />
                    <span className="min-w-0 break-words text-sm leading-relaxed text-gray-600">
                  ...
                  <>
                    ...
                    <div className="sm:col-span-2">
                      <input
                        ...
                        onChange={handleChange}
                            className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 transition-colors placeholder-gray-400 focus:outline-none"
                      />
                    </div>
                  </>
                )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {(['YAPE', 'PLIN', 'EFECTIVO', 'TRANSFERENCIA', 'TARJETA', 'MERCADO_PAGO'] as MedioPago[]).map(method => {
                  ...
                  return (
                    <label
                      key={method}
                      className="flex min-w-0 cursor-pointer items-center gap-2.5 rounded-2xl border-2 px-3 py-3 text-[13px] font-bold transition-all sm:px-4 sm:text-sm"
                      style={active
                        ? { borderColor: cp, background: `${cp}10`, color: cp }
                        : { borderColor: '#E5E7EB', color: '#6B7280' }
                      }
                    >
                      ...
                      <Icon icon={meta.icon} width={18} className="flex-shrink-0" />
                ...
                <div className="mt-6 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {configPago.cuentasBancarias.map((cuenta: any) => (
                      <div key={cuenta.id} className="flex min-w-0 flex-col gap-3 rounded-2xl border border-gray-200 bg-gray-50 p-4 sm:flex-row sm:items-center sm:gap-4">
                        <BancoLogo banco={cuenta.banco} size={48} />
                        <div className="flex min-w-0 flex-1 flex-col gap-1">
                          <span className="break-all font-mono text-sm font-bold tracking-tight text-gray-800">{cuenta.numeroCuenta}</span>
            ...
            <div className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={3}
                className="w-full resize-none rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 transition-colors placeholder-gray-400 focus:outline-none"
              />
            </div>
          ...
          <div className="w-full space-y-4 lg:sticky lg:top-28 lg:w-[340px] lg:flex-shrink-0">
            ...
            <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
              ...
              <div className="px-5 py-3 space-y-3 max-h-56 overflow-y-auto border-b border-gray-100">
                {carrito.map(item => (
                  <div key={item.id} className="flex min-w-0 items-center gap-3">
                    ...
                    <p className="line-clamp-2 min-w-0 flex-1 break-words text-xs font-semibold leading-tight text-gray-600">{item.descripcion}</p>
                    <span className="flex-shrink-0 whitespace-nowrap text-xs font-black text-gray-900">
              ...
              <div className="px-6 py-5 space-y-3">
                ...
                <div className="flex items-end justify-between gap-3 border-t border-gray-100 pt-4">
                  ...
                  <span className="whitespace-nowrap text-2xl font-black" style={{ color: cp }}>
                    S/ {total.toFixed(2)}
                  </span>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              ...
              <div className="flex flex-wrap gap-2">
                {(['YAPE', 'PLIN', 'EFECTIVO'] as MedioPago[]).map(m => {
      ...
      {/* ── Footer ── */}
      <MayeFooter tienda={tienda} diseno={diseno} slug={slug} />
    </motion.div>
  );
}
