...
import { URB, UrbCartModal, UrbFooter, UrbHeader, UrbProductCard, UrbProductImage, UrbWhatsAppFab, urbFont, urbPrimary, waLink, withAlpha } from '@/templates/ropa-hombre/RopaHombreParts';
import { urbCard, urbFade, urbPage, urbSection, urbStagger, urbTap, urbViewport } from '@/templates/ropa-hombre/motion';
...
export function RopaHombreProductoDetalleView({
  tienda,
  slug,
  producto,
  related = [],
  ...
  const primary = urbPrimary(cp || diseno?.colorPrimario);
  ...
  const desc = producto?.detalle || producto?.descripcionCorta ||
    'Una prenda cuidadosamente confeccionada con telas premium. Corte atemporal, acabados finos y comodidad pensada para tu día a día.';
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={urbPage} className="min-h-screen" style={{ backgroundColor: URB.cream, fontFamily: font }}>
      ...
      {/* Breadcrumb */}
      <div className="border-b" style={{ borderColor: URB.line, background: `linear-gradient(120% 120% at 85% 0%, ${URB.nude}, ${URB.cream} 60%)` }}>
      ...
      <main className="mx-auto max-w-7xl px-6 py-10 md:py-14">
        <div className="grid gap-10 lg:grid-cols-2">
          {/* Galería */}
          <motion.div variants={urbFade}>
            <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border" style={{ backgroundColor: URB.mist, borderColor: URB.line }}>
              {gallery[activeImage] ? (
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {gallery.slice(0, 5).map((src, i) => (
                  <button
                    ...
                    style={{ backgroundColor: URB.mist, borderColor: i === activeImage ? URB.ink : 'transparent' }}
                  >
          ...
          {/* Info */}
          <motion.div variants={urbSection}>
            {marca && <p className="text-[11px] font-semibold uppercase tracking-[0.22em]" style={{ color: URB.gold }}>{marca}</p>}
            <h1 className="mt-2 text-4xl leading-tight md:text-5xl" style={{ fontFamily: URB.serif, color: URB.ink }}>{name}</h1>

            <div className="mt-4 flex items-center gap-3">
              <span style={{ color: URB.gold }}>{'★'.repeat(stars)}<span className="text-neutral-300">{'★'.repeat(5 - stars)}</span></span>
              <span className="text-sm text-neutral-400">{categoria || 'Ropa de hombre'}</span>
            </div>

            <div className="mt-6 flex items-baseline gap-3">
              <span className="text-4xl font-semibold" style={{ fontFamily: URB.serif, color: URB.ink }}>{money(pricing.precioFinal)}</span>
            ...
            <div className="mt-6 flex items-center gap-2 text-sm">
              <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${outOfStock ? 'bg-neutral-100 text-neutral-500' : 'text-white'}`} style={outOfStock ? undefined : { backgroundColor: URB.ink }}>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <div className="inline-flex h-[52px] items-center rounded-full border bg-white" style={{ borderColor: URB.line }}>
              ...
              <motion.button
                ...
                whileTap={outOfStock ? undefined : urbTap}
            ...
            <motion.button
              ...
              style={{ borderColor: URB.ink, color: URB.ink }}
            >
              Comprar ahora
            </motion.button>

            {/* Beneficios */}
            <div className="mt-8 grid gap-4 border-t pt-8 sm:grid-cols-3" style={{ borderColor: URB.line }}>
              {[
                ['solar:hanger-2-linear', 'Calidad premium'],
                ['solar:refresh-linear', 'Cambios fáciles'],
                ['solar:box-minimalistic-linear', 'Envío a domicilio'],
              ].map(([icon, text]) => (
                <div key={text} className="flex items-center gap-2.5 text-xs text-neutral-600">
                  <Icon icon={icon} width={20} style={{ color: URB.ink }} />
                  {text}
                </div>
              ))}
            </div>
            ...
            {/* Compartir */}
            <div className="mt-8 flex items-center gap-3 border-t pt-6" style={{ borderColor: URB.line }}>
              ...
              {['mdi:whatsapp', 'mdi:facebook', 'mdi:instagram'].map((ic) => (
                <a key={ic} href={ic === 'mdi:whatsapp' ? waLink(tienda, `Hola, me interesa: ${name}`) : '#'} target="_blank" rel="noreferrer" className="grid h-9 w-9 place-items-center rounded-full border text-neutral-500 transition-colors hover:text-neutral-900" style={{ borderColor: URB.line, backgroundColor: withAlpha(primary, '0d') }}>
        ...
        {/* Descripción larga */}
        <motion.section initial="hidden" whileInView="show" viewport={urbViewport} variants={urbSection} className="mt-14 rounded-2xl border bg-white p-8 md:p-10" style={{ borderColor: URB.line }}>
          <h2 className="text-2xl" style={{ fontFamily: URB.serif, color: URB.ink }}>Sobre esta prenda</h2>
          ...
          <div className="mt-6 grid gap-3 text-sm sm:grid-cols-2">
            <p><span className="font-semibold" style={{ color: URB.ink }}>Categoría:</span> <span className="text-neutral-500">{categoria || '—'}</span></p>
            <p><span className="font-semibold" style={{ color: URB.ink }}>Marca:</span> <span className="text-neutral-500">{marca || '—'}</span></p>
            <p><span className="font-semibold" style={{ color: URB.ink }}>SKU:</span> <span className="text-neutral-500">{producto?.codigo || producto?.sku || '—'}</span></p>
            <p><span className="font-semibold" style={{ color: URB.ink }}>Disponibilidad:</span> <span className="text-neutral-500">{outOfStock ? 'Agotado' : `${stock} unidades`}</span></p>
          </div>
        </motion.section>

        {/* Relacionados */}
        {relatedFiltered.length > 0 && (
          <motion.section initial="hidden" whileInView="show" viewport={urbViewport} variants={urbSection} className="mt-16">
            <div className="mb-8 text-center">
              <p className="text-[11px] font-semibold uppercase tracking-[0.3em]" style={{ color: URB.gold }}>Descubre más</p>
              <h2 className="mt-2 text-3xl uppercase tracking-[0.1em]" style={{ fontFamily: URB.serif, color: URB.ink }}>Combina tu look</h2>
            </div>
            <motion.div variants={urbStagger} className="grid grid-cols-2 gap-x-6 gap-y-10 lg:grid-cols-4">
              {relatedFiltered.map((p) => (
                <UrbProductCard key={p.id} producto={p} slug={slug} cp={primary} onAddToCart={onAddToCart || ((prod) => actualizarCantidad(prod.id, 1))} onClick={() => go(`/tienda/${slug}/producto/${p.id}`, 'producto')} />
              ))}
            </motion.div>
          </motion.section>
        )}
...
export default function RopaHombreProductoDetalle() {
