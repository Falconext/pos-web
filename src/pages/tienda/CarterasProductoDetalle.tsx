...
export function CarterasProductoDetalleView({
  tienda,
  slug,
  producto,
  related = [],
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={luxPage} className="min-h-screen" style={{ backgroundColor: LUX.cream, fontFamily: font }}>
      ...
      {/* Breadcrumb */}
      <div className="border-b" style={{ borderColor: LUX.line, background: `linear-gradient(120% 120% at 85% 0%, ${LUX.nude}, ${LUX.cream} 60%)` }}>
      ...
      <main className="mx-auto max-w-7xl px-6 py-10 md:py-14">
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <motion.div variants={luxFade}>
            <div className="relative aspect-square overflow-hidden rounded-2xl border" style={{ backgroundColor: LUX.mist, borderColor: LUX.line }}>
              {gallery[activeImage] ? (
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {gallery.slice(0, 5).map((src, i) => (
                  <button
                    ...
                    style={{ backgroundColor: LUX.mist, borderColor: i === activeImage ? LUX.ink : 'transparent' }}
                  >
          ...
          <motion.div variants={luxSection}>
            {marca && <p className="text-[11px] font-semibold uppercase tracking-[0.22em]" style={{ color: LUX.gold }}>{marca}</p>}
            <h1 className="mt-2 text-4xl leading-tight md:text-5xl" style={{ fontFamily: LUX.serif, color: LUX.ink }}>{name}</h1>

            <div className="mt-4 flex items-center gap-3">
              <span style={{ color: LUX.gold }}>{'★'.repeat(stars)}<span className="text-neutral-300">{'★'.repeat(5 - stars)}</span></span>
              <span className="text-sm text-neutral-400">{categoria || 'Accesorios'}</span>
            </div>

            <div className="mt-6 flex items-baseline gap-3">
              <span className="text-4xl font-semibold" style={{ fontFamily: LUX.serif, color: LUX.ink }}>{money(pricing.precioFinal)}</span>
            ...
            <div className="mt-6 flex items-center gap-2 text-sm">
              <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${outOfStock ? 'bg-neutral-100 text-neutral-500' : 'text-white'}`} style={outOfStock ? undefined : { backgroundColor: LUX.ink }}>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <div className="inline-flex h-[52px] items-center rounded-full border bg-white" style={{ borderColor: LUX.line }}>
            ...
            {/* Beneficios */}
            <div className="mt-8 grid gap-4 border-t pt-8 sm:grid-cols-3" style={{ borderColor: LUX.line }}>
              {[
                ['solar:crown-line-linear', 'Calidad premium'],
                ...
                <div key={text} className="flex items-center gap-2.5 text-xs text-neutral-600">
                  <Icon icon={icon} width={20} style={{ color: LUX.ink }} />
                  {text}
                </div>
              ))}
            </div>
            ...
            {/* Compartir */}
            <div className="mt-8 flex items-center gap-3 border-t pt-6" style={{ borderColor: LUX.line }}>
              ...
              {['mdi:whatsapp', 'mdi:facebook', 'mdi:instagram'].map((ic) => (
                <a key={ic} href={ic === 'mdi:whatsapp' ? waLink(tienda, `Hola, me interesa: ${name}`) : '#'} target="_blank" rel="noreferrer" className="grid h-9 w-9 place-items-center rounded-full border text-neutral-500 transition-colors hover:text-neutral-900" style={{ borderColor: LUX.line, backgroundColor: withAlpha(primary, '0d') }}>
        ...
        {/* Descripción larga */}
        <motion.section initial="hidden" whileInView="show" viewport={luxViewport} variants={luxSection} className="mt-14 rounded-2xl border bg-white p-8 md:p-10" style={{ borderColor: LUX.line }}>
          <h2 className="text-2xl" style={{ fontFamily: LUX.serif, color: LUX.ink }}>Sobre este producto</h2>
          ...
          <div className="mt-6 grid gap-3 text-sm sm:grid-cols-2">
            <p><span className="font-semibold" style={{ color: LUX.ink }}>Categoría:</span> <span className="text-neutral-500">{categoria || '—'}</span></p>
            <p><span className="font-semibold" style={{ color: LUX.ink }}>Marca:</span> <span className="text-neutral-500">{marca || '—'}</span></p>
            <p><span className="font-semibold" style={{ color: LUX.ink }}>SKU:</span> <span className="text-neutral-500">{producto?.codigo || producto?.sku || '—'}</span></p>
            <p><span className="font-semibold" style={{ color: LUX.ink }}>Disponibilidad:</span> <span className="text-neutral-500">{outOfStock ? 'Agotado' : `${stock} unidades`}</span></p>
          </div>
        </motion.section>
        ...
        {relatedFiltered.length > 0 && (
          <motion.section initial="hidden" whileInView="show" viewport={luxViewport} variants={luxSection} className="mt-16">
            <div className="mb-8 text-center">
              ...
              <h2 className="mt-2 text-3xl uppercase tracking-[0.1em]" style={{ fontFamily: LUX.serif, color: LUX.ink }}>También te encantará</h2>
            </div>
            <motion.div variants={luxStagger} className="grid grid-cols-2 gap-x-6 gap-y-10 lg:grid-cols-4">
              {relatedFiltered.map((p) => (
                <LuxProductCard key={p.id} producto={p} slug={slug} cp={primary} onAddToCart={onAddToCart || ((prod) => actualizarCantidad(prod.id, 1))} onClick={() => go(`/tienda/${slug}/producto/${p.id}`, 'producto')} />
              ))}
            </motion.div>
          </motion.section>
        )}
...
export default function CarterasProductoDetalle() {
