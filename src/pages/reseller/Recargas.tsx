...
export default function ResellerRecargas() {
    ...
    useEffect(() => {
        if (auth?.resellerId) {
            ...
            getDashboard(auth.resellerId); // Ensure stats are fresh
        }
    }, [auth]);
    ...
    return (
        <div className="space-y-6 animate-in fade-in zoom-in duration-300">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                {/* Optional: Add "Solicitar Recarga" button later if needed */}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-2xl font-bold text-gray-800">S/ {Number(stats.saldo).toFixed(2)}</p>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600">
                        <Icon icon="solar:tag-price-bold" width="24" />
                    </div>
                    <div>
                        <p className="text-sm text-gray-500 font-medium">Nivel de Descuento</p>
                        <p className="text-2xl font-bold text-gray-800">{stats.porcentajeDescuento}%</p>
                    </div>
                </div>
            </div>
                                ...
                                <tr>
                                    <td className="px-6 py-12" colSpan={5}>
                                        <div className="flex flex-col items-center justify-center text-gray-400">
                                            <Icon icon="solar:card-search-linear" width="48" className="mb-2 opacity-50" />
                                ...
                                recargas.map((recarga: any) => (
                                    <tr key={recarga.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-gray-800">
                                            {moment(recarga.fecha).format('DD MMM YYYY, hh:mm A')}
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="font-bold text-emerald-600">+S/ {Number(recarga.monto).toFixed(2)}</span>
                                        </td>
                                        <td className="px-6 py-4 text-gray-600">
                                            {recarga.referencia || '-'}
                                        </td>
                                        <td className="px-6 py-4">
                                            {/* Assuming all showed here are completed, or verify status field if exists. Schema says just fecha, monto, referencia usually. */}
