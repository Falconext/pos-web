...
import { LUX, LuxuryFooter, LuxuryHeader, LuxuryProductImage, luxFont, luxPrimary } from './LuxuryParts';
...
export default function LuxuryCheckoutPage(props: TemplateCheckoutPageProps) {
  const {
    slug,
    tienda,
    diseno,
  ...
  const ocultarEnvio = Boolean(diseno?.luxuryOcultarEnvio);
  ...
  return (
    <motion.div
      ...
      style={{ backgroundColor: LUX.porcelain, fontFamily: font, ['--lux-cp' as any]: primary }}
    >
      <LuxuryHeader
        tienda={tienda}
        slug={slug}
      ...
      <section className="relative overflow-hidden border-b border-black/[0.06]" style={{ background: `radial-gradient(120% 120% at 80% 0%, ${primary}22, ${LUX.porcelain} 60%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-12 text-center md:py-14">
          ...
          <h1 className="mt-3 text-4xl text-neutral-900 md:text-5xl" style={{ fontFamily: LUX.serif }}>Finaliza tu pedido</h1>
        </div>
      </section>
      ...
      <motion.main variants={luxSection} className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_430px]">
          <section className="space-y-8">
            {/* Paso 1 — datos */}
            <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxCard} className="rounded-2xl border border-black/[0.06] bg-white p-6 md:p-8">
              <div className="mb-7 flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="text-2xl text-neutral-900" style={{ fontFamily: LUX.serif }}>Tus datos</h2>
                </div>
              </div>
            ...
            <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxCard} className="rounded-2xl border border-black/[0.06] bg-white p-6 md:p-8">
              <div className="mb-7 flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="text-2xl text-neutral-900" style={{ fontFamily: LUX.serif }}>Entrega y pago</h2>
                </div>
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <label className="block">
                  ...
                  <select name="tipoEntrega" value={deliveryType} onChange={handleChange} className={inputClass(erroresForm.tipoEntrega)}>
                    {configEnvio?.aceptaRecojo !== false && <option value="RECOJO">Recojo en boutique</option>}
            ...
            <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxStagger} className="grid gap-4 md:grid-cols-3">
              {[
                ['solar:verified-check-linear', 'Originales', 'Fragancias auténticas y selladas.'],
                ['solar:gift-linear', 'Envoltura de regalo', 'Presentación de lujo sin costo.'],
                ['solar:chat-round-dots-linear', 'Asesoría', 'Te acompañamos por WhatsApp.'],
              ].map(([icon, title, text]) => (
                <motion.div key={title} variants={luxCard} className="rounded-2xl border border-black/[0.06] bg-white p-6">
            ...
            {suggestedProducts.length > 0 && (
              <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxSection} className="rounded-2xl border border-black/[0.06] bg-white p-6 md:p-8">
                <div className="mb-6 flex items-end justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em]" style={{ color: LUX.gold }}>Completa tu ritual</p>
                    <h2 className="text-2xl text-neutral-900" style={{ fontFamily: LUX.serif }}>También te encantará</h2>
                  </div>
                  <a href={`/tienda/${slug}/catalogo`} className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-900 hover:opacity-70">Ver todo</a>
                </div>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {suggestedProducts.slice(0, 3).map((producto) => (
                    <button
                      ...
                      className="group flex items-center gap-3 rounded-2xl border border-black/[0.06] p-3 text-left transition-colors hover:border-neutral-900/20"
                    >
                      <span className="h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-[#F6F2FB]">
                        <LuxuryProductImage producto={producto} />
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="line-clamp-2 text-sm font-medium text-neutral-900">{producto.descripcion}</span>
          ...
          {/* Resumen */}
          <motion.aside initial="hidden" animate="show" variants={luxCard} className="h-fit overflow-hidden rounded-2xl border border-black/[0.06] bg-white shadow-[0_30px_60px_-40px_rgba(21,17,28,0.5)] lg:sticky lg:top-28">
            <div className="border-b border-neutral-100 p-6">
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-neutral-400">Tu pedido</p>
              <h2 className="mt-1 text-2xl text-neutral-900" style={{ fontFamily: LUX.serif }}>Resumen</h2>
            </div>
            ...
            <div className="max-h-[440px] overflow-y-auto px-6">
              {carritoState.length === 0 ? (
                <div className="my-6 rounded-2xl bg-neutral-50 p-8 text-center">
                  ...
                  <a href={`/tienda/${slug}/catalogo`} className="mt-5 inline-flex rounded-full px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-white" style={{ backgroundColor: primary }}>Ver colección</a>
                </div>
              ) : (
                carritoState.map((item) => {
                  ...
                  return (
                    <div key={itemId} className="flex gap-4 border-b border-neutral-100 py-5">
                      <div className="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-[#F6F2FB]">
                        <LuxuryProductImage producto={item} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="line-clamp-2 text-sm font-medium leading-5 text-neutral-900">{item.descripcion}</p>
            ...
            {!ocultarEnvio && freeDeliveryRemaining > 0 && carritoState.length > 0 && (
              <div className="mx-6 mt-5 rounded-xl p-4 text-xs font-medium leading-5 text-neutral-800" style={{ backgroundColor: `${primary}12` }}>
            ...
            <div className="p-6">
              <div className="space-y-3 text-sm">
                <div className="flex justify-between text-neutral-500"><span>Subtotal</span><span className="font-medium text-neutral-900">{money(calcularSubtotal())}</span></div>
                {!ocultarEnvio && <div className="flex justify-between text-neutral-500"><span>Envío</span><span className="font-medium text-neutral-900">{money(calcularCostoEnvio())}</span></div>}
                <div className="flex items-baseline justify-between border-t border-neutral-100 pt-4">
                  ...
                  <span className="text-3xl font-semibold text-neutral-900" style={{ fontFamily: LUX.serif }}>{money(ocultarEnvio ? calcularSubtotal() : calcularTotal())}</span>
                </div>
              </div>
      ...
      <LuxuryFooter tienda={tienda} slug={slug} diseno={diseno} cp={primary} categories={[]} />
