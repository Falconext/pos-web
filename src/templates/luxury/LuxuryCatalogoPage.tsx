...
import { LUX, LuxuryFooter, LuxuryHeader, LuxuryProductCard, luxFont, luxPrimary } from './LuxuryParts';
...
const OLFACTIVE = ['Floral', 'Amaderado', 'Cítrico', 'Oriental', 'Fresco', 'Dulce'];

function SidebarBox({ title, children }: { title: string; children: ReactNode }) {
  return (
    <motion.div variants={luxCard} initial="hidden" whileInView="show" viewport={luxViewport} className="rounded-2xl border border-black/[0.06] bg-white p-6">
      <h3 className="mb-5 border-b border-neutral-100 pb-4 text-[13px] font-semibold uppercase tracking-[0.16em] text-neutral-900">{title}</h3>
      {children}
    </motion.div>
  );
}

export default function LuxuryCatalogoPage({
  tienda,
  slug,
  diseno,
  cp,
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={luxPage} className="min-h-screen" style={{ backgroundColor: LUX.porcelain, fontFamily: font }}>
      ...
      <section className="relative overflow-hidden border-b border-black/[0.06]" style={{ background: `radial-gradient(120% 120% at 80% 0%, ${primary}22, ${LUX.porcelain} 60%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-12 text-center md:py-16">
          <div className="text-xs font-medium uppercase tracking-[0.2em] text-neutral-400">
            ...
            <span className="text-neutral-700">Colección</span>
          </div>
          <h1 className="mt-3 text-4xl text-neutral-900 md:text-5xl" style={{ fontFamily: LUX.serif }}>Nuestra colección</h1>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6 pt-8">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              style={selectedCategorías.length === 0 ? { backgroundColor: primary, color: '#fff' } : { backgroundColor: '#fff', color: LUX.ink, border: '1px solid rgba(0,0,0,0.1)' }}
            >
              Todo
            </button>
            {categoryList.slice(0, 10).map((cat: any, index: number) => {
              ...
              return (
                <button
                  ...
                  className="rounded-full px-4 py-2 text-xs font-medium uppercase tracking-[0.12em] transition-colors"
                  style={active ? { backgroundColor: primary, color: '#fff' } : { backgroundColor: '#fff', color: LUX.ink, border: '1px solid rgba(0,0,0,0.1)' }}
                >
                  {name}
                </button>
              );
      ...
      <motion.main variants={luxSection} className="mx-auto max-w-7xl px-6 py-10">
        <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
          <aside className="space-y-6 lg:sticky lg:top-28 lg:h-fit">
                  ...
                  {brandList.slice(0, 10).map((brand: any, index: number) => {
                    ...
                    return (
                      <label key={`${name}-${index}`} className="flex cursor-pointer items-center gap-3 text-sm text-neutral-600">
                        <span className="flex h-5 w-5 items-center justify-center rounded-md border border-neutral-300 bg-white" style={checked ? { backgroundColor: primary, borderColor: primary } : undefined}>
            ...
            <SidebarBox title="Familia olfativa">
              <div className="flex flex-wrap gap-2">
                {OLFACTIVE.map((fam) => (
                  <button key={fam} type="button" onClick={() => setSearch(fam.toLowerCase())} className="rounded-full border border-neutral-200 px-3 py-1.5 text-xs font-medium text-neutral-600 transition-colors hover:border-neutral-900 hover:text-neutral-900">
                    {fam}
                  </button>
                ))}
              </div>
          ...
          <section className="min-w-0">
            <div className="mb-8 flex flex-col justify-between gap-4 rounded-2xl border border-black/[0.06] bg-white p-4 md:flex-row md:items-center">
              <p className="px-2 text-sm text-neutral-500">
                <span className="font-semibold text-neutral-900">{sortedProductos.length}</span> de <span className="font-semibold text-neutral-900">{totalLabel}</span> fragancias
              </p>
              <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="h-11 min-w-[210px] rounded-full border border-neutral-200 bg-neutral-50 px-5 text-sm font-medium text-neutral-600 outline-none">
            ...
            {loading ? (
              <div className="grid gap-6 grid-cols-2 xl:grid-cols-3">
                {Array.from({ length: 9 }).map((_, index) => <div key={index} className="h-[420px] animate-pulse rounded-2xl bg-black/[0.04]" />)}
              </div>
            ) : sortedProductos.length === 0 ? (
              <motion.div variants={luxCard} initial="hidden" animate="show" className="rounded-2xl border border-black/[0.06] bg-white p-16 text-center">
                <Icon icon="mdi:bottle-tonic-plus-outline" className="mx-auto mb-4 text-6xl" style={{ color: primary }} />
                <h3 className="text-xl text-neutral-900" style={{ fontFamily: LUX.serif }}>No encontramos esa fragancia</h3>
                <p className="mt-2 text-sm text-neutral-500">Prueba con otra familia olfativa o limpia los filtros.</p>
                <button type="button" onClick={clearFilters} className="mt-6 rounded-full px-6 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-white" style={{ backgroundColor: primary }}>Limpiar filtros</button>
              </motion.div>
            ) : (
              <>
                <AnimatePresence mode="wait">
                  <motion.div key="grid" variants={luxStagger} initial="hidden" animate="show" exit={{ opacity: 0, y: 12, transition: { duration: 0.18 } }} className="grid gap-6 grid-cols-2 xl:grid-cols-3">
                    {sortedProductos.map((producto) => (
                      <LuxuryProductCard key={producto.id} producto={producto} slug={slug} cp={primary} onAddToCart={handleAgregarProducto} onClick={() => navigate(`/tienda/${slug}/producto/${producto.id}`)} />
                    ))}
                  </motion.div>
                </AnimatePresence>
                {productos.length < total && (
                  <div className="mt-12 flex justify-center">
                    <button type="button" onClick={() => cargarProductos(page + 1)} className="rounded-full px-8 py-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-white shadow-lg" style={{ backgroundColor: primary }}>
                      Cargar más
                    </button>
                  </div>
                )}
      ...
      <LuxuryFooter tienda={tienda} slug={slug} diseno={diseno} cp={primary} categories={allCategorías} />
