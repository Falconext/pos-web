...
import { SPA, SpaFooter, SpaHeader, SpaProductCard, SpaWhatsAppFab, spaFont, spaPrimary } from './SpaParts';
import { spaCard, spaPage, spaSection, spaStagger, spaViewport } from './motion';
...
function SidebarBox({ title, children }: { title: string; children: ReactNode }) {
  return (
    <motion.div variants={spaCard} initial="hidden" whileInView="show" viewport={spaViewport} className="rounded-3xl border border-black/[0.05] bg-white p-6">
      <h3 className="mb-5 border-b border-neutral-100 pb-4 text-[13px] font-semibold uppercase tracking-[0.16em]" style={{ color: SPA.ink }}>{title}</h3>
      {children}
    </motion.div>
  );
}

export default function SpaCatalogoPage({
  tienda,
  slug,
  diseno,
  cp,
    ...
    <motion.div initial="hidden" animate="show" variants={spaPage} className="min-h-screen" style={{ backgroundColor: SPA.cream, fontFamily: font }}>
      ...
      <section className="relative overflow-hidden border-b border-black/[0.05]" style={{ background: `radial-gradient(120% 120% at 80% 0%, ${primary}22, ${SPA.cream} 60%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-12 text-center md:py-16">
          ...
          <h1 className="mt-3 text-4xl md:text-5xl" style={{ fontFamily: SPA.serif, color: SPA.ink }}>Nuestra tienda</h1>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6 pt-8">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              style={selectedCategorías.length === 0 ? { backgroundColor: primary, color: '#fff' } : { backgroundColor: '#fff', color: SPA.ink, border: '1px solid rgba(0,0,0,0.1)' }}
            >
              Todo
            </button>
            {categoryList.slice(0, 10).map((cat: any, index: number) => {
              ...
              return (
                <button
                  ...
                  className="rounded-full px-4 py-2 text-xs font-medium uppercase tracking-[0.12em] transition-colors"
                  style={active ? { backgroundColor: primary, color: '#fff' } : { backgroundColor: '#fff', color: SPA.ink, border: '1px solid rgba(0,0,0,0.1)' }}
                >
                  {name}
                </button>
              );
      ...
      <motion.main variants={spaSection} className="mx-auto max-w-7xl px-6 py-10">
        <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
          <aside className="space-y-6 lg:sticky lg:top-28 lg:h-fit">
            ...
            <SidebarBox title="Precio">
              ...
              />
              {hasActiveFilters && (
                <button type="button" onClick={clearFilters} className="mt-5 rounded-full px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-white" style={{ backgroundColor: SPA.espresso }}>
                  Limpiar filtros
                </button>
              )}
            </SidebarBox>
          ...
          <section className="min-w-0">
            <div className="mb-8 flex flex-col justify-between gap-4 rounded-3xl border border-black/[0.05] bg-white p-4 md:flex-row md:items-center">
              <p className="px-2 text-sm text-neutral-500">
                <span className="font-semibold" style={{ color: SPA.ink }}>{sortedProductos.length}</span> de <span className="font-semibold" style={{ color: SPA.ink }}>{totalLabel}</span> productos
              </p>
            ...
            {loading ? (
              <div className="grid grid-cols-2 gap-6 xl:grid-cols-3">
                {Array.from({ length: 9 }).map((_, index) => <div key={index} className="h-[380px] animate-pulse rounded-3xl bg-black/[0.04]" />)}
              </div>
            ) : sortedProductos.length === 0 ? (
              <motion.div variants={spaCard} initial="hidden" animate="show" className="rounded-3xl border border-black/[0.05] bg-white p-16 text-center">
                <Icon icon="mdi:spa-outline" className="mx-auto mb-4 text-6xl" style={{ color: primary }} />
                <h3 className="text-xl" style={{ fontFamily: SPA.serif, color: SPA.ink }}>No encontramos ese producto</h3>
              ...
              <>
                <AnimatePresence mode="wait">
                  <motion.div key="grid" variants={spaStagger} initial="hidden" animate="show" exit={{ opacity: 0, y: 12, transition: { duration: 0.18 } }} className="grid grid-cols-2 gap-6 xl:grid-cols-3">
                    {sortedProductos.map((producto) => (
                      <SpaProductCard key={producto.id} producto={producto} slug={slug} cp={primary} onAddToCart={handleAgregarProducto} onClick={() => navigate(`/tienda/${slug}/producto/${producto.id}`)} />
                    ))}
                  </motion.div>
                </AnimatePresence>
      ...
      <SpaFooter tienda={tienda} slug={slug} diseno={diseno} cp={primary} categories={allCategorías} />
