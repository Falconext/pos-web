...
import { SPA, SpaFooter, SpaHeader, SpaWhatsAppFab, spaFont, spaPrimary, waLink, withAlpha } from './SpaParts';
import { spaCard, spaPage, spaSection, spaStagger, spaTap, spaViewport } from './motion';

function pickContact(tienda: any, diseno: any) {
  return {
    address: diseno?.spaContactAddress || tienda?.direccionTienda || tienda?.direccionFiscal || tienda?.direccion || '',
    phone: diseno?.spaContactPhone || tienda?.whatsappTienda || tienda?.telefono || tienda?.celular || '',
    email: diseno?.spaContactEmail || tienda?.email || tienda?.correo || '',
    hours: diseno?.spaContactHours || tienda?.horarioAtencion || '',
  };
}

export default function SpaContactPage({
  tienda,
  slug,
  diseno,
  cp,
  ...
  const displayHours = contact.hours || 'Lun a Sáb · 9:00 – 20:00';
  ...
  const inputCls = 'mt-2 h-12 w-full rounded-xl border border-neutral-200 bg-white px-4 text-sm text-neutral-900 outline-none transition-colors focus:border-[var(--spa-cp)]';

  return (
    <motion.div initial="hidden" animate="show" variants={spaPage} className="min-h-screen" style={{ backgroundColor: SPA.cream, fontFamily: font, ['--spa-cp' as any]: primary }}>
      <SpaHeader
        tienda={tienda}
        slug={slug}
        cp={primary}
      ...
      <section className="relative overflow-hidden border-b border-black/[0.05]" style={{ background: `radial-gradient(120% 120% at 80% 0%, ${primary}22, ${SPA.cream} 60%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-12 text-center md:py-16">
          ...
          <h1 className="mt-3 text-4xl md:text-5xl" style={{ fontFamily: SPA.serif, color: SPA.ink }}>Reserva tu momento de belleza</h1>
          <p className="mx-auto mt-3 max-w-md text-sm text-neutral-500">¿Quieres agendar una cita o consultar un tratamiento? Nuestro equipo te asesora con gusto.</p>
        </div>
      </section>

      <motion.main variants={spaSection} className="mx-auto max-w-7xl px-6 py-14">
        <div className="grid gap-8 lg:grid-cols-[1.4fr_0.9fr]">
          <motion.div variants={spaCard} className="min-h-[420px] overflow-hidden rounded-3xl border border-black/[0.05] md:min-h-[560px]">
            <iframe
          ...
          <motion.form
            variants={spaCard}
            className="rounded-3xl border border-black/[0.05] bg-white p-7 md:p-9"
            onSubmit={(event) => { event.preventDefault(); setSent(true); }}
          >
            <h2 className="text-2xl" style={{ fontFamily: SPA.serif, color: SPA.ink }}>{diseno?.spaContactTitle || 'Escríbenos'}</h2>
            ...
            <label className="mt-5 block text-[11px] font-semibold uppercase tracking-[0.14em] text-neutral-500">
              Tu mensaje
              <textarea rows={5} className="mt-2 w-full resize-none rounded-xl border border-neutral-200 bg-white p-4 text-sm text-neutral-900 outline-none transition-colors focus:border-[var(--spa-cp)]" />
            </label>
            {sent && (
            ...
            <motion.button
              ...
              whileTap={spaTap}
            >
              {diseno?.spaContactSubmitLabel || 'Enviar mensaje'}
            </motion.button>
            <a
              ...
              style={{ borderColor: SPA.nude, color: SPA.espresso }}
            >
        ...
        <motion.section initial="hidden" whileInView="show" viewport={spaViewport} variants={spaStagger} className="mt-10 grid gap-4 md:grid-cols-4">
          {[
            ...
            <motion.div key={`${label}-${index}`} variants={spaCard} className="rounded-3xl border border-black/[0.05] bg-white p-6">
              <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ background: `linear-gradient(135deg, ${primary}, ${SPA.espresso})` }}>
                <Icon icon={icon} width={20} />
              </span>
              <p className="mt-4 text-[11px] font-semibold uppercase tracking-[0.14em]" style={{ color: SPA.gold }}>{label}</p>
