...
import { URB, UrbFooter, UrbHeader, UrbProductImage, UrbWhatsAppFab, urbFont, urbPrimary } from './RopaHombreParts';
...
function inputClass(hasError?: boolean) {
  return `mt-2 h-[52px] w-full rounded-xl border bg-white px-4 text-sm text-neutral-900 outline-none transition-colors ${
    hasError ? 'border-red-400' : 'border-neutral-200 focus:border-[var(--urb-cp)]'
  }`;
}

export default function RopaHombreCheckoutPage(props: TemplateCheckoutPageProps) {
  const {
    slug,
    tienda,
    diseno,
  ...
  const ocultarEnvio = Boolean(diseno?.ropaHombreOcultarEnvio);
  ...
  return (
    <motion.div
      ...
      style={{ backgroundColor: URB.cream, fontFamily: font, ['--urb-cp' as any]: URB.ink }}
    >
      <UrbHeader
        tienda={tienda}
        slug={slug}
      ...
      <section className="relative overflow-hidden border-b" style={{ borderColor: URB.line, background: `linear-gradient(120% 120% at 80% 0%, ${URB.nude}, ${URB.cream} 60%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-12 text-center md:py-14">
          ...
          <h1 className="mt-3 text-4xl uppercase tracking-[0.1em] md:text-5xl" style={{ fontFamily: URB.serif, color: URB.ink }}>Finaliza tu pedido</h1>
        </div>
      </section>

      <motion.main variants={urbSection} className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_430px]">
          <section className="space-y-8">
            {/* Paso 1 — datos */}
            <motion.div initial="hidden" whileInView="show" viewport={urbViewport} variants={urbCard} className="rounded-2xl border bg-white p-6 md:p-8" style={{ borderColor: URB.line }}>
              <div className="mb-7 flex items-center gap-3">
                <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ backgroundColor: URB.ink }}>
                ...
                <div>
                  ...
                  <h2 className="text-2xl" style={{ fontFamily: URB.serif, color: URB.ink }}>Tus datos</h2>
                </div>
              </div>
            ...
            <motion.div initial="hidden" whileInView="show" viewport={urbViewport} variants={urbCard} className="rounded-2xl border bg-white p-6 md:p-8" style={{ borderColor: URB.line }}>
              <div className="mb-7 flex items-center gap-3">
                <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ backgroundColor: URB.charcoal }}>
                  <Icon icon="solar:delivery-linear" width={22} style={{ color: URB.goldSoft }} />
                </span>
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-neutral-400">Paso 2</p>
                  <h2 className="text-2xl" style={{ fontFamily: URB.serif, color: URB.ink }}>Entrega y pago</h2>
                </div>
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <label className="block">
                ...
                <label className="block md:col-span-2">
                  ...
                  <textarea name="observaciones" value={formData.observaciones || ''} onChange={handleChange} rows={3} className="mt-2 w-full resize-none rounded-xl border border-neutral-200 bg-white px-4 py-4 text-sm text-neutral-900 outline-none transition-colors focus:border-[var(--urb-cp)]" placeholder="¿Talla, color o alguna referencia? Déjanos una nota..." />
                </label>
              </div>
            </motion.div>

            <motion.div initial="hidden" whileInView="show" viewport={urbViewport} variants={urbStagger} className="grid gap-4 md:grid-cols-3">
              {[
                ['solar:hanger-2-linear', 'Calidad premium', 'Telas seleccionadas para durar.'],
                ['solar:refresh-linear', 'Cambios fáciles', 'Cambia tu talla sin complicaciones.'],
                ['solar:chat-round-dots-linear', 'Asesoría', 'Te acompañamos por WhatsApp.'],
              ].map(([icon, title, text]) => (
                <motion.div key={title} variants={urbCard} className="rounded-2xl border bg-white p-6" style={{ borderColor: URB.line }}>
                  <Icon icon={icon} width={26} style={{ color: URB.ink }} />
                  <p className="mt-4 text-sm font-semibold" style={{ color: URB.ink }}>{title}</p>
            ...
            {suggestedProducts.length > 0 && (
              <motion.div initial="hidden" whileInView="show" viewport={urbViewport} variants={urbSection} className="rounded-2xl border bg-white p-6 md:p-8" style={{ borderColor: URB.line }}>
                <div className="mb-6 flex items-end justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em]" style={{ color: URB.gold }}>Completa tu look</p>
                    <h2 className="text-2xl" style={{ fontFamily: URB.serif, color: URB.ink }}>También te quedará bien</h2>
                  </div>
                  <a href={`/tienda/${slug}/catalogo`} className="text-xs font-semibold uppercase tracking-[0.14em] hover:opacity-70" style={{ color: URB.ink }}>Ver todo</a>
                </div>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {suggestedProducts.slice(0, 3).map((producto) => (
                    ...
                    >
                      <span className="h-16 w-16 shrink-0 overflow-hidden rounded-xl" style={{ backgroundColor: URB.mist }}>
                        <UrbProductImage producto={producto} />
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="line-clamp-2 text-sm font-medium" style={{ color: URB.ink }}>{producto.descripcion}</span>
                        <span className="mt-1 block text-sm font-semibold" style={{ color: URB.ink }}>{money(producto.precioUnitario)}</span>
                      </span>
          ...
          {/* Resumen */}
          <motion.aside initial="hidden" animate="show" variants={urbCard} className="h-fit overflow-hidden rounded-2xl border bg-white shadow-[0_30px_60px_-40px_rgba(26,22,19,0.5)] lg:sticky lg:top-28" style={{ borderColor: URB.line }}>
            <div className="border-b p-6" style={{ borderColor: URB.line }}>
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-neutral-400">Tu pedido</p>
              <h2 className="mt-1 text-2xl" style={{ fontFamily: URB.serif, color: URB.ink }}>Resumen</h2>
            </div>
            ...
            <div className="max-h-[440px] overflow-y-auto px-6">
              {carritoState.length === 0 ? (
                <div className="my-6 rounded-2xl bg-neutral-50 p-8 text-center">
                  ...
                  <a href={`/tienda/${slug}/catalogo`} className="mt-5 inline-flex rounded-full px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-white" style={{ backgroundColor: URB.ink }}>Ver tienda</a>
                </div>
              ) : (
            ...
            {!ocultarEnvio && freeDeliveryRemaining > 0 && carritoState.length > 0 && (
              <div className="mx-6 mt-5 rounded-xl p-4 text-xs font-medium leading-5" style={{ backgroundColor: URB.mist, color: URB.ink }}>
            ...
            <div className="p-6">
              <div className="space-y-3 text-sm">
                <div className="flex justify-between text-neutral-500"><span>Subtotal</span><span className="font-medium" style={{ color: URB.ink }}>{money(calcularSubtotal())}</span></div>
                {!ocultarEnvio && <div className="flex justify-between text-neutral-500"><span>Envío</span><span className="font-medium" style={{ color: URB.ink }}>{money(calcularCostoEnvio())}</span></div>}
                <div className="flex items-baseline justify-between border-t pt-4" style={{ borderColor: URB.line }}>
                  <span className="text-sm font-semibold uppercase tracking-[0.12em] text-neutral-500">Total</span>
                  <span className="text-3xl font-semibold" style={{ fontFamily: URB.serif, color: URB.ink }}>{money(ocultarEnvio ? calcularSubtotal() : calcularTotal())}</span>
                </div>
              </div>
              ...
              <motion.button
                ...
                whileTap={canSubmit ? urbTap : undefined}
              >
      ...
      <UrbFooter tienda={tienda} slug={slug} diseno={diseno} cp={primary} categories={[]} />
