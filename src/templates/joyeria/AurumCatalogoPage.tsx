...
import { aurCard, aurPage, aurSection, aurStagger, aurViewport } from './motion';
...
function SidebarBox({ title, children }: { title: string; children: ReactNode }) {
  return (
    <motion.div variants={aurCard} initial="hidden" whileInView="show" viewport={aurViewport} className="rounded-2xl border bg-white p-6" style={{ borderColor: AUR.line }}>
      <h3 className="mb-5 border-b pb-4 text-[12px] font-semibold uppercase tracking-[0.18em]" style={{ color: AUR.ink, borderColor: AUR.line }}>{title}</h3>
      {children}
    </motion.div>
  );
}

export default function AurumCatalogoPage({
  tienda,
  slug,
  diseno,
  cp,
    ...
    <motion.div initial="hidden" animate="show" variants={aurPage} className="min-h-screen" style={{ backgroundColor: AUR.cream, fontFamily: font }}>
      ...
      <section className="relative overflow-hidden border-b" style={{ borderColor: AUR.line, background: `linear-gradient(120% 120% at 80% 0%, ${AUR.nude}, ${AUR.cream} 60%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-12 text-center md:py-16">
          ...
          <h1 className="mt-3 text-5xl font-bold lowercase tracking-tight md:text-6xl" style={{ fontFamily: AUR.serif, color: AUR.ink }}>nuestra colección</h1>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6 pt-8">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              style={selectedCategorías.length === 0 ? { backgroundColor: AUR.ink, color: '#fff' } : { backgroundColor: '#fff', color: AUR.ink, border: `1px solid ${AUR.line}` }}
            >
              Todo
            </button>
            {categoryList.slice(0, 10).map((cat: any, index: number) => {
              ...
              return (
                <button
                  ...
                  className="rounded-full px-4 py-2 text-xs font-medium uppercase tracking-[0.12em] transition-colors"
                  style={active ? { backgroundColor: AUR.ink, color: '#fff' } : { backgroundColor: '#fff', color: AUR.ink, border: `1px solid ${AUR.line}` }}
                >
                  {name}
                </button>
              );
      ...
      <motion.main variants={aurSection} className="mx-auto max-w-7xl px-6 py-10">
        <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
          <aside className="space-y-6 lg:sticky lg:top-28 lg:h-fit">
                  ...
                  {brandList.slice(0, 10).map((brand: any, index: number) => {
                    ...
                    return (
                      <label key={`${name}-${index}`} className="flex cursor-pointer items-center gap-3 text-sm text-neutral-600">
                        <span className="flex h-5 w-5 items-center justify-center rounded-md border bg-white" style={checked ? { backgroundColor: AUR.ink, borderColor: AUR.ink } : { borderColor: AUR.tan }}>
            ...
            <SidebarBox title="Precio">
              ...
              <input
                ...
                className="w-full"
                style={{ accentColor: AUR.ink }}
              />
              {hasActiveFilters && (
                <button type="button" onClick={clearFilters} className="mt-5 rounded-full px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-white" style={{ backgroundColor: AUR.ink }}>
                  Limpiar filtros
                </button>
              )}
            </SidebarBox>
          ...
          <section className="min-w-0">
            <div className="mb-8 flex flex-col justify-between gap-4 rounded-2xl border bg-white p-4 md:flex-row md:items-center" style={{ borderColor: AUR.line }}>
              <p className="px-2 text-sm text-neutral-500">
                <span className="font-semibold" style={{ color: AUR.ink }}>{sortedProductos.length}</span> de <span className="font-semibold" style={{ color: AUR.ink }}>{totalLabel}</span> joyas
              </p>
              <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="h-11 min-w-[210px] rounded-full border bg-neutral-50 px-5 text-sm font-medium text-neutral-600 outline-none" style={{ borderColor: AUR.line }}>
            ...
            {loading ? (
              ...
              <motion.div variants={aurCard} initial="hidden" animate="show" className="rounded-2xl border bg-white p-16 text-center" style={{ borderColor: AUR.line }}>
                <Icon icon="solar:diamond-linear" className="mx-auto mb-4 text-6xl" style={{ color: AUR.tan }} />
                <h3 className="text-2xl font-bold lowercase" style={{ fontFamily: AUR.serif, color: AUR.ink }}>no encontramos esa joya</h3>
                <p className="mt-2 text-sm text-neutral-500">Prueba con otra categoría o limpia los filtros.</p>
                <button type="button" onClick={clearFilters} className="mt-6 rounded-full px-6 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-white" style={{ backgroundColor: AUR.ink }}>Limpiar filtros</button>
              </motion.div>
            ) : (
              <>
                <AnimatePresence mode="wait">
                  <motion.div key="grid" variants={aurStagger} initial="hidden" animate="show" exit={{ opacity: 0, y: 12, transition: { duration: 0.18 } }} className="grid grid-cols-2 gap-x-6 gap-y-10 xl:grid-cols-3">
                    {sortedProductos.map((producto) => (
                      <AurProductCard key={producto.id} producto={producto} slug={slug} cp={primary} onAddToCart={handleAgregarProducto} onClick={() => navigate(`/tienda/${slug}/producto/${producto.id}`)} />
                    ))}
                  </motion.div>
                </AnimatePresence>
                {productos.length < total && (
                  <div className="mt-12 flex justify-center">
                    <button type="button" onClick={() => cargarProductos(page + 1)} className="rounded-full px-8 py-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-white shadow-lg" style={{ backgroundColor: AUR.ink }}>
                      Cargar más
                    </button>
                  </div>
                )}
      ...
      <AurFooter tienda={tienda} slug={slug} diseno={diseno} cp={primary} categories={allCategorías} />
