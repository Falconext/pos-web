...
import { LUX, LuxFooter, LuxHeader, LuxProductImage, LuxWhatsAppFab, luxFont, luxPrimary } from './CarterasParts';
...
function inputClass(hasError?: boolean) {
  return `mt-2 h-[52px] w-full rounded-xl border bg-white px-4 text-sm text-neutral-900 outline-none transition-colors ${
    hasError ? 'border-red-400' : 'border-neutral-200 focus:border-[var(--lux-cp)]'
  }`;
}

export default function CarterasCheckoutPage(props: TemplateCheckoutPageProps) {
  const {
    slug,
    tienda,
    diseno,
  ...
  const ocultarEnvio = Boolean(diseno?.carterasOcultarEnvio);
  ...
  return (
    <motion.div
      ...
      style={{ backgroundColor: LUX.cream, fontFamily: font, ['--lux-cp' as any]: LUX.ink }}
    >
      <LuxHeader
        tienda={tienda}
        slug={slug}
      ...
      <section className="relative overflow-hidden border-b" style={{ borderColor: LUX.line, background: `linear-gradient(120% 120% at 80% 0%, ${LUX.nude}, ${LUX.cream} 60%)` }}>
        <div className="mx-auto max-w-7xl px-6 py-12 text-center md:py-14">
          ...
          <h1 className="mt-3 text-4xl uppercase tracking-[0.1em] md:text-5xl" style={{ fontFamily: LUX.serif, color: LUX.ink }}>Finaliza tu pedido</h1>
        </div>
      </section>

      <motion.main variants={luxSection} className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_430px]">
          <section className="space-y-8">
            {/* Paso 1 — datos */}
            <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxCard} className="rounded-2xl border bg-white p-6 md:p-8" style={{ borderColor: LUX.line }}>
              <div className="mb-7 flex items-center gap-3">
                <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ backgroundColor: LUX.ink }}>
                ...
                <div>
                  ...
                  <h2 className="text-2xl" style={{ fontFamily: LUX.serif, color: LUX.ink }}>Tus datos</h2>
                </div>
              </div>
            ...
            <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxCard} className="rounded-2xl border bg-white p-6 md:p-8" style={{ borderColor: LUX.line }}>
              <div className="mb-7 flex items-center gap-3">
                <span className="flex h-11 w-11 items-center justify-center rounded-full text-white" style={{ backgroundColor: LUX.charcoal }}>
                  <Icon icon="solar:delivery-linear" width={22} style={{ color: LUX.goldSoft }} />
                </span>
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-neutral-400">Paso 2</p>
                  <h2 className="text-2xl" style={{ fontFamily: LUX.serif, color: LUX.ink }}>Entrega y pago</h2>
                </div>
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <label className="block">
                ...
                <label className="block md:col-span-2">
                  ...
                  <textarea name="observaciones" value={formData.observaciones || ''} onChange={handleChange} rows={3} className="mt-2 w-full resize-none rounded-xl border border-neutral-200 bg-white px-4 py-4 text-sm text-neutral-900 outline-none transition-colors focus:border-[var(--lux-cp)]" placeholder="¿Es para regalo? Déjanos una dedicatoria o referencia..." />
                </label>
              </div>
            </motion.div>

            <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxStagger} className="grid gap-4 md:grid-cols-3">
              {[
                ['solar:crown-line-linear', 'Calidad premium', 'Materiales seleccionados a mano.'],
                ['solar:gift-linear', 'Listo para regalar', 'Empaque cuidado sin costo.'],
                ['solar:chat-round-dots-linear', 'Asesoría', 'Te acompañamos por WhatsApp.'],
              ].map(([icon, title, text]) => (
                <motion.div key={title} variants={luxCard} className="rounded-2xl border bg-white p-6" style={{ borderColor: LUX.line }}>
                  <Icon icon={icon} width={26} style={{ color: LUX.ink }} />
                  <p className="mt-4 text-sm font-semibold" style={{ color: LUX.ink }}>{title}</p>
            ...
            {suggestedProducts.length > 0 && (
              <motion.div initial="hidden" whileInView="show" viewport={luxViewport} variants={luxSection} className="rounded-2xl border bg-white p-6 md:p-8" style={{ borderColor: LUX.line }}>
                <div className="mb-6 flex items-end justify-between gap-4">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em]" style={{ color: LUX.gold }}>Completa tu look</p>
                    <h2 className="text-2xl" style={{ fontFamily: LUX.serif, color: LUX.ink }}>También te encantará</h2>
                  </div>
                  <a href={`/tienda/${slug}/catalogo`} className="text-xs font-semibold uppercase tracking-[0.14em] hover:opacity-70" style={{ color: LUX.ink }}>Ver todo</a>
                </div>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {suggestedProducts.slice(0, 3).map((producto) => (
                    ...
                    >
                      <span className="h-16 w-16 shrink-0 overflow-hidden rounded-xl" style={{ backgroundColor: LUX.mist }}>
                        <LuxProductImage producto={producto} />
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="line-clamp-2 text-sm font-medium" style={{ color: LUX.ink }}>{producto.descripcion}</span>
                        <span className="mt-1 block text-sm font-semibold" style={{ color: LUX.ink }}>{money(producto.precioUnitario)}</span>
                      </span>
          ...
          {/* Resumen */}
          <motion.aside initial="hidden" animate="show" variants={luxCard} className="h-fit overflow-hidden rounded-2xl border bg-white shadow-[0_30px_60px_-40px_rgba(26,22,19,0.5)] lg:sticky lg:top-28" style={{ borderColor: LUX.line }}>
            <div className="border-b p-6" style={{ borderColor: LUX.line }}>
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-neutral-400">Tu pedido</p>
              <h2 className="mt-1 text-2xl" style={{ fontFamily: LUX.serif, color: LUX.ink }}>Resumen</h2>
            </div>
            ...
            <div className="max-h-[440px] overflow-y-auto px-6">
              {carritoState.length === 0 ? (
                <div className="my-6 rounded-2xl bg-neutral-50 p-8 text-center">
                  ...
                  <a href={`/tienda/${slug}/catalogo`} className="mt-5 inline-flex rounded-full px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-white" style={{ backgroundColor: LUX.ink }}>Ver tienda</a>
                </div>
              ) : (
            ...
            {!ocultarEnvio && freeDeliveryRemaining > 0 && carritoState.length > 0 && (
              <div className="mx-6 mt-5 rounded-xl p-4 text-xs font-medium leading-5" style={{ backgroundColor: LUX.mist, color: LUX.ink }}>
            ...
            <div className="p-6">
              <div className="space-y-3 text-sm">
                <div className="flex justify-between text-neutral-500"><span>Subtotal</span><span className="font-medium" style={{ color: LUX.ink }}>{money(calcularSubtotal())}</span></div>
                {!ocultarEnvio && <div className="flex justify-between text-neutral-500"><span>Envío</span><span className="font-medium" style={{ color: LUX.ink }}>{money(calcularCostoEnvio())}</span></div>}
                <div className="flex items-baseline justify-between border-t pt-4" style={{ borderColor: LUX.line }}>
                  <span className="text-sm font-semibold uppercase tracking-[0.12em] text-neutral-500">Total</span>
                  <span className="text-3xl font-semibold" style={{ fontFamily: LUX.serif, color: LUX.ink }}>{money(ocultarEnvio ? calcularSubtotal() : calcularTotal())}</span>
                </div>
              </div>
              ...
              <motion.button
                ...
                whileTap={canSubmit ? luxTap : undefined}
              >
      ...
      <LuxFooter tienda={tienda} slug={slug} diseno={diseno} cp={primary} categories={[]} />
